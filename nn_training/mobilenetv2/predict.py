...
from .config import CLASS_NAMES, DEFAULT_CHECKPOINT_DIR, TrainConfig
...
def load_checkpoint_model(checkpoint_path: Path, device: torch.device):
    ...
    model = build_model(num_classes=len(class_names), dropout=dropout, pretrained=False).to(device)
