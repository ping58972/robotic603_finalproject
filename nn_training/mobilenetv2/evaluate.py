...
from .config import CLASS_NAMES, DEFAULT_CHECKPOINT_DIR, DEFAULT_DATA_DIR, DEFAULT_RUNS_DIR, TrainConfig
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Evaluate a trained symbol MobileNetV2 checkpoint.")
    parser.add_argument("--checkpoint", type=Path, default=DEFAULT_CHECKPOINT_DIR / "symbols_mobilenetv2_best.pt")
...
def main() -> None:
    ...
    model = build_model(num_classes=len(class_names), dropout=0.0, pretrained=False).to(device)
